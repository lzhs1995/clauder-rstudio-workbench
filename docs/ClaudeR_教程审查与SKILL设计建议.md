...
审查日期：2026-09-07。本文是版本化审查与整改台账，不是发布成功报告。
当前安装：workbench v0.6.0 + 连接诊断补丁；v0.6.1 候选尚未正式发布。
本轮状态：用户因 Claude 欠费明确授权 Codex 独立接管；不再调用 Claude，也不声称有本轮联合终审。配套 ClaudeR 已完成三平台 CI、PR 合并；工作台整改、持续观测和发布验收正在收尾。
...
| 对象 | 本次核查基线 |
|---|---|
| 上游 main | 8a3227179f0bd5b55fe7c8f080f2a719d37339e2 |
...
- 当前 workbench 完整 Python 测试 `254/254`；新增 readiness 14 项、配置存储 8 项，覆盖换进程恢复、错误 PID、配置改变、归档篡改、显式禁用及并发冲突。后续版本锁定测试另列最终验收。
